...
def index(request):
    try:
        ...
        return render(
            request,
            "nanuapp/index.html",
            {"active_page": "home", "video_url": video_url},
        )
    except:
        raise Http404()
...
def standard_project(request):
    try:
        return render(
            request, "nanuapp/standard-project.html", {"active_page": "portfolio"}
        )
    except:
        raise Http404()
...
def contact(request):
    try:
        if request.method == "POST":
            ...
            form = ContactForm()
        return render(
            request, "nanuapp/contact.html", {"active_page": "contact", "form": form}
        )
